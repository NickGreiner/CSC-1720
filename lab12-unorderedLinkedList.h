...
public:
    ...
    //Postcondition: Smallest node in the list will be
    //	       removed and deleted from the list.
};
...
void unorderedLinkedList<Type>::deleteSmallest()
{
    ...
    {
        smallest = this->first;
        testSmallest = this->first;

        while (testSmallest != NULL)
        {
            if (testSmallest->info < smallest->info)
                smallest = testSmallest;
            
            testSmallest = testSmallest->link;
        }
