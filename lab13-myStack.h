...
public:
    ...
    void reverseStack(stackType<Type> &otherStack);
      //Function to copy existing stack into another existing
...
{
    int counter = stackTop - 1;   //int to retrieve list info set
                                  //to one behind stackTop

    otherStack.initializeStack();   //clears otherStack

    for (int i = 0; i < stackTop; i++)
    {
        otherStack.push(list[counter]);
        counter--;
    }
}//end reverseStack
